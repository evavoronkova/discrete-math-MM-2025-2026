...
KS = [5]  # количество ориентиров
STRATEGIES = ["random", "degree", "coverage"]
NUM_PAIRS = 10  # сколько случайных пар вершин тестируем
    ...
    for filepath in all_files:
        ...
        for k in KS:
            for strat in STRATEGIES:
                ...
                # Общие аргументы для from_strategy
                strat_kwargs = {"seed": 40}
                if strat == "coverage":
                    strat_kwargs["M"] = COVERAGE_M

                # LandmarksBasic (создание + предобработка)
                t0 = time.time()
                lb = LandmarksBasic.from_strategy(g, k, strategy=strat, **strat_kwargs)
                ...
                # LandmarksSC (создание + предобработка)
                t0 = time.time()
                lsc = LandmarksSC.from_strategy(g, k, strategy=strat, **strat_kwargs)
